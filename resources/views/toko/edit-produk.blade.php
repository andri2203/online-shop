...
<form action="{{ route('store.ubah_produk', ['productID'=>$produk['id']]) }}" enctype="multipart/form-data" method="post" class="grid grid-cols-2 rounded-lg w-full px-4 py-2.5 bg-gray-800 gap-y-2 gap-x-4">
    @csrf
    <div class="col-span-2 row-span-3" x-data="{ preview: '{{ asset('images/' . $produk['photo']) }}', change:false }">
        <label for="cover-photo" class="block text-sm/6 font-medium text-gray-400">Cover photo</label>
                ...
                <div class="mt-4 flex justify-center text-sm/6 text-gray-400">
                    <label for="file-upload"
                        ...
                        <input id="file-upload" name="photo" type="file" class="sr-only"
                            @change="const file = $event.target.files[0]; preview = file ? URL.createObjectURL(file) : null; change = true" />
                    </label>
                </div>
